...
/**
 * @brief Lookup handler for named string properties; currently unimplemented and yields no value.
 *
 * Prints a diagnostic message indicating the requested key and returns an empty string.
 *
 * @param what Key or name of the requested string property (unused).
...
/**
 * @brief Register a streaming input with the luminosity counting manager.
 *
 * Associates the provided streaming input with this manager, configures it for
 * streaming operation, and registers it for the specified subsystem. For GL1
 * inputs the DST node is created only when not running in streaming-only mode;
 * valid GL1 inputs are stored for later processing.
 *
 * @param evtin Pointer to the streaming input to register.
 * @param system Subsystem identifier specifying how the input is handled (e.g., GL1).
 *
 * @note If an unsupported subsystem is provided, the function prints an error and terminates the process.
 */
