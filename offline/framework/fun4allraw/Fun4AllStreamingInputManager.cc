...
/**
 * @brief Populate the INTT raw-hit container from cached INTT input pools and update INTT QA state.
 *
 * Ensures INTT input pools are filled (via FillInttPool), locates or creates the InttRawHitContainer node,
 * establishes or updates the reference BCO window, discards cached INTT entries that are earlier than the
 * allowed negative offset (invoking CleanupUsedPackets / clearPacketBClkStackMap / clearFeeGTML1BCOMap on each
 * registered INTT input for any discarded BCO), fills QA histograms that tag packets/fees relative to the current
 * reference BCO, and appends all INTT hits with BCO <= (m_RefBCO + m_intt_bco_range) into the InttRawHitContainer.
 * The function may call gSystem->Exit(1) and abort if the InttRawHitContainer node cannot be found or created.
 *
 * @return int `0` on success, or the non-zero error code returned by FillInttPool().
 */
...
/**
 * @brief Populate MVTX raw event header and hit container from buffered MVTX packets.
 *
 * Computes a BCO selection window based on the current reference BCO and configured MVTX ranges,
 * performs tagging and QA histogram updates, and transfers matching FeeIdInfo, L1 trigger BCOs,
 * and raw hits from the internal MVTX buffers into the MVTX raw event header and MVTX raw hit
 * container nodes in the top node tree.
 *
 * The function initializes the reference BCO if it is unset, advances past buffered entries that
 * are earlier than the allowed negative offset, and only moves entries whose BCO falls within the
 * computed lower and upper limits. Missing required MVTX nodes cause an immediate abort.
 *
 * @return int 0 on success, non-zero error code returned by FillMvtxPool on failure.
 */
...
/**
 * @brief Ensures MVTX input pools are filled and validates run number consistency.
 *
 * Calls FillPool(ref_bco_minus_range) on each registered MVTX input, sets the manager run
 * number from the first input, and aborts the process if a run-number mismatch is detected.
 *
 * @return int `0` on success, `-1` if the MVTX raw-hit map is empty (no data available).
 */
