 ...
 * @return T The value multiplied by itself.
 */
inline T square (T const& t) { return t * t; }
  ...
  class range_adaptor
  {
   public:
     ...
     *
     * Stores the provided begin and end iterators so the adaptor can expose a
     * range-compatible interface via begin() and end().
     *
    ...
    /**
 * @brief Access the range's beginning iterator.
 *
 * @return const reference to the stored begin iterator.
 */
T const& begin() { return m_begin; }
    /**
 * @brief Returns the stored end iterator of the range adaptor.
 *
 * @return T const& Const reference to the stored end iterator.
 */
 ...
 * @brief Construct a StateClusterResidualsQA module with the given instance name.
 *
 * @param name Identifier used to register the SubsysReco module and label produced QA objects.
 */
...
/**
 * @brief Initialize run: create histograms and retrieve required nodes and histograms.
 *
 * Retrieves SvtxTrackMap, TrkrClusterContainer, ActsGeometry, and the QA histogram
 * manager from the provided top-level node. If any of these cannot be found the
 * run is aborted. On success, internal vectors of histogram pointers are populated
 * for each pending configuration.
 *
 * @param top_node Top-level node of the event/data tree.
 * @return int Fun4All return code: `EVENT_OK` on success; `ABORTRUN` if a required
 *         node (track map, cluster container, ActsGeometry) or the QA histogram
...
/**
 * @brief Processes a single event and fills per-configuration residual histograms between track states and clusters.
 *
 * For each track, counts clusters per detector subsystem, applies each pending configuration's selection
 * (charge sign, per-subsystem cluster-count ranges, phi/eta/pt ranges), and for configurations that pass
 * fills the X, Y, and Z histograms with the difference (state position - cluster global position) for every
 * state associated with a cluster.
 *
 * @param top_node Top-level event node used to retrieve the SvtxTrackMap, TrkrClusterContainer, and ActsGeometry.
...
/**
 * @brief Create and register per-configuration histograms for state–cluster residuals.
 *
 * For each configuration in m_pending, creates three TH1F histograms named
 * "<cfg.name>_x", "<cfg.name>_y", and "<cfg.name>_z" for the X, Y, and Z
 * residuals respectively. Each histogram uses m_nBins and the corresponding
 * axis range (m_xrange, m_yrange, m_zrange), is labeled with "State-Cluster
 * <X|Y|Z> Residual [cm]" on the x-axis and "Entries" on the y-axis, is styled
 * with blue marker and line colors, and is registered with the QA histogram
 * manager.
 */
...
/**
 * @brief Finalize the module at the end of a run.
 *
 * Ensures the QA histogram manager is available and performs any finalization
 * steps required by this QA module.
 *
