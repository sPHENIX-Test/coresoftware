...
namespace
{
  ...
  class range_adaptor
  {
   public:
    /**
     * @brief Construct a range_adaptor that provides begin/end access for a pair-like range.
     *
     * Wraps the provided range by reference so callers can iterate over the range's first and
     * second elements without copying the underlying range.
     *
     * @param range The pair-like range to adapt; referenced (not copied) by the adaptor.
     */
    ...
    /**
 * @brief Accesses the beginning element of the adapted range.
 *
 * @return const typename T::first_type& Const reference to the range's first element.
 */
const typename T::first_type& begin() { return m_range.first; }
    /**
 * @brief Access the second element of the wrapped range.
 *
 * @return const typename T::second_type& The second element of the underlying pair (typically the end iterator or upper bound of the range).
 */
const typename T::second_type& end() { return m_range.second; }
  ...
  /**
   * @brief Prints basic identifying information about an MVTX cluster to standard output.
   *
   * When `cluster` is non-null, prints the cluster key, local (x,y) position, size,
   * layer, stave id, chip id, strobe id, and cluster index. When `cluster` is null,
   * prints the cluster key along with layer, stave id, chip id, strobe id, and index.
   *
   * @param ckey The TrkrDefs::cluskey identifying the cluster.
   * @param cluster Pointer to the TrkrCluster; may be null to indicate only key-derived fields should be printed.
   */
...
/**
 * @brief Constructs an MvtxClusterPruner SubsysReco module.
 *
 * @param name Name assigned to this module instance and forwarded to SubsysReco.
 */
...
/**
 * @brief Initialize per-run state and report the configured matching mode.
 *
 * Prints the current value of m_use_strict_matching to standard output.
 *
 * @return int Fun4All return code: `Fun4AllReturnCodes::EVENT_OK`.
 */
...
/**
 * @brief Prunes MVTX clusters by comparing clusters across consecutive strobe steps.
 *
 * Compares clusters in each MVTX hitset with clusters from the next strobe and removes
 * redundant clusters according to the configured matching mode: exact match (strict)
 * or subset-based (removes the cluster whose hit set is a subset of the other).
 *
 * @param topNode Root node of the event node tree used to access cluster containers.
...
/**
 * @brief Print final cluster pruning statistics.
 *
 * Outputs the total number of clusters processed, the number of clusters deleted,
 * and the deleted fraction to standard output.
 *
 * @return Fun4AllReturnCodes::EVENT_OK
 */
