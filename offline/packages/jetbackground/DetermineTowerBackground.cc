...
/**
 * @brief Initialize the run and set up output nodes, optionally loading average calorimeter v2 calibrations.
 *
 * If flow mode equals 4, attempts to load average calorimeter v2 calibrations before node creation;
 * if calibration loading fails, the run initialization is aborted.
 *
 * @param topNode Top-level node from which to create or attach output nodes.
 * @return int Fun4All return code: `Fun4AllReturnCodes::ABORTRUN` if calibration loading failed when required,
 * otherwise the result of creating the output node (e.g., `EVENT_OK` on success).
 */
...
/**
 * @brief Load calo average v2 calibration and populate centrality v2 values.
 *
 * Loads calibration data from the configured CDB URL (or the overwrite path when set)
 * and fills the internal _CENTRALITY_V2 vector with 100 values taken from the
 * "jet_calo_v2" field for centrality bins 0–99.
 *
 * If no calibration path is found, the function prints an error message and terminates
...
/**
 * @brief Compute per-layer underlying-event (UE) tower densities and event flow (Psi2, v2) and store results in the node tree.
 *
 * Extracts tower and jet information from the provided node tree, selects seed regions, computes flow (from calo, sEPD, HIJING truth, or centrality depending on configuration), applies seed and bad-tower masking and flow modulation, calculates UE density per eta strip for EMCAL/IHCAL/OHCAL, and writes the results to the TowerBackground node.
 *
 * @param topNode Top-level PHCompositeNode containing detector, jet, eventplane, and centrality nodes required for the calculation.
 * @return int Fun4AllReturnCodes::EVENT_OK on success;
 *              Fun4AllReturnCodes::ABORTRUN if required tower information is missing;
 *              -1 for fatal failures during flow extraction (e.g., missing truth or eventplane when required).
 */
