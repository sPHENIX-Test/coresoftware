...
namespace
{
  ...
  /**
   * @brief Computes the contribution of a layer model for a discrete bin.
   *
   * Calculates the overlap between a unit-width bin centered at round(x[0]) and the interval
   * [mu - 0.5, mu + 0.5], then returns A multiplied by that overlap. Returns 0 if there is no overlap.
   *
   * @param x Input array where x[0] is the (possibly non-integer) layer index to be binned.
   * @param par Parameter array where par[0] = A (amplitude) and par[1] = mu (layer center).
   * @return double A times the overlap length between the binned layer and the layer interval, or 0.0 if none.
   */
  ...
  /**
   * @brief Evaluates a scaled Gaussian for the phi coordinate.
   *
   * Computes par[0] multiplied by a Gaussian evaluated at x[0] with mean par[1] and width par[2].
   *
   * @param x Pointer to the independent variable array; x[0] is the value at which to evaluate the Gaussian.
   * @param par Parameter array where par[0] is the amplitude scale, par[1] is the Gaussian mean, and par[2] is the Gaussian sigma.
   * @return double The value par[0] * Gaussian(x[0]; mean=par[1], sigma=par[2]); returns 0.0 if par[2] < 0.0.
   */
  ...
  /**
   * @brief Models the time-dependent amplitude as a Gaussian modulated by a CDF-like term.
   *
   * Evaluates a Gaussian centered at par[1] with width par[2], multiplies it by
   * (1 + erfc(par[3]*(x[0]-par[1])/(sqrt(2)*par[2]))) to introduce an asymmetric tail,
   * and scales the result by par[0].
   *
   * @param x Input array where x[0] is the time coordinate to evaluate.
   * @param par Parameter array interpreted as:
   *   - par[0]: overall amplitude scale,
   *   - par[1]: Gaussian mean,
   *   - par[2]: Gaussian standard deviation (if < 0 the function returns 0),
   *   - par[3]: coefficient controlling the CDF-like asymmetry.
   * @return double Computed value: par[0] * Gaussian(x[0]; par[1], par[2]) * (1 + erfc(par[3]*(x[0]-par[1])/(sqrt(2)*par[2]))); returns 0 if par[2] < 0.
   */
  ...
  /**
   * @brief Reduce a set of candidate hits to the connected region seeded by a specified hit.
   *
   * Performs a 3D adjacency flood-fill (using the module's neighborOffsets) to find contiguous hits
   * connected to the seed hit identified by maxKey, then replaces clusHits with the hits from that
   * connected region. If the seed key is not found in the input, the first hit is used as the seed.
   *
   * @param clusHits Input vector of hitData candidates; on return it contains only the connected region
   *                 (one contiguous cluster) grown from the seed.
   * @param maxKey   Pair of (hitkey, hitsetkey) used to identify the seed hit within clusHits.
   */
  ...
  /**
   * @brief Removes a set of hits from the spatial index and corresponding ADC map entries.
   *
   * For each entry in clusHits, removes that hit from the R-tree and erases the first
   * matching entry in adcMap whose stored spechitkey equals the hit's spechitkey.
   *
   * @param clusHits Vector of hit entries to remove; each element's spechitkey is used to find ADC map matches.
   * @param rtree R-tree spatial index from which the hits in clusHits will be removed.
   * @param adcMap Multimap of ADC entries; the first entry with a matching spechitkey is erased for each removed hit.
   */
  ...
  /**
   * @brief Build and record a laser cluster from a set of candidate hits.
   *
   * Processes the provided hit list (first reducing it to the connected region
   * around the supplied seed), computes centroid and weighted moments, optionally
   * performs a 3D likelihood fit to refine layer/phi/time parameters, and
   * constructs a LaserClusterv2 which is appended to the per-thread cluster
   * storage in my_data.
   *
   * The function allocates a LaserClusterv2, may allocate a temporary TH3D and
   * a ROOT::Fit::Fitter for fitting, and always pushes the created cluster and
   * its generated cluster key into my_data.cluster_vector and
   * my_data.cluster_key_vector respectively.
   *
   * @param clusHits Vector of candidate hits; will be reduced to the connected
   *                 region around maxADCKey before cluster calculation.
   *                 Each entry is expected to provide (layer, iphi, it) coords,
   *                 an ADC value, and the original hit keys.
   * @param my_data  Per-thread processing context and outputs. Used for geometry,
   *                 thresholds, fitting flag, and to receive the created cluster
   *                 and cluster key via my_data.cluster_vector and
   *                 my_data.cluster_key_vector.
   * @param maxADCKey Pair of (hitkey, hitsetkey) identifying the seed hit with
   *                  maximum ADC used to define the connected region.
   */
  ...
  /**
   * @brief Processes TPC hits for a single thread partition and produces laser clusters.
   *
   * Collects hits from the thread's assigned hitsets that pass the configured ADC threshold
   * and peak-time-bin filter, deduplicates spatially, and iteratively builds clusters starting
   * from the highest-ADC remaining hit. For each cluster, computes cluster parameters (via
   * calc_cluster_parameter) and removes clustered hits so they are not reused. Appends resulting
   * LaserClusterv2 objects and their keys into the provided thread_data (cluster_vector and
   * cluster_keys).
   *
   * @param my_data Per-thread state and configuration (input hitsets, geometry, thresholds) and
   *                output locations for generated clusters and their keys.
  ...
  /**
   * @brief Thread entry point that processes a module's data and then terminates the thread.
   *
   * @param threadarg Pointer to a thread_data structure containing per-thread state; must be a pointer to thread_data.
   * @return void* Thread exit status: always `nullptr`.
   */
 ...
 * @brief Construct a LaserClusterizer subsystem with the given name.
 *
 * Initializes the LaserClusterizer as a SubsysReco module and associates it with the provided identifier.
 *
 * @param name Identifier used to register and reference this subsystem instance.
 */
...
/**
 * @brief Initialize runtime resources and ensure required nodes are present.
 *
 * Ensures the DST tree contains a Laser cluster container (or lamination variant) and that the TPC geometry container
 * is available; initializes the ADC clock period and maximum drift time used by the clusterizer.
 *
 * @param topNode Top-level node of the PHENIX/Fun4All node tree from which required subnodes are located or created.
 * @return int Fun4AllReturnCodes::EVENT_OK on success, Fun4AllReturnCodes::ABORTRUN if required nodes (DST or TPCGEOMCONTAINER)
 * cannot be found.
 */
...
/**
 * @brief Process a single event: locate required nodes, build laser clusters, and store them in the cluster container.
 *
 * Traverses the node tree starting at topNode to read EventHeader, LaserEventInfo, hitsets, geometry, and the target
 * LaserCluster container. If the event is a laser event, spawns worker threads for each sector/side/module to
 * collect hits, form clusters, optionally fit cluster parameters, and add resulting LaserCluster objects to the
 * LaserClusterContainer.
 *
 ...
 *             Fun4AllReturnCodes::ABORTRUN if required nodes (EventHeader, LaserEventInfo, TRKR_HITSET, cluster node,
 *             or ActsGeometry) are missing;
 *             non-zero on lower-level initialization or thread creation/join failures.
 */
