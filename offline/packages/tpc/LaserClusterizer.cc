...
namespace
{
  ...
  /**
   * @brief Computes the expected layer response as the overlap fraction between a unit-width bin centered at x[0] and a peak centered at `mu`, scaled by amplitude `A`.
   *
   * The function treats the peak as occupying a unit interval centered at `mu` and the measurement as a unit interval centered at round(x[0]); it returns the length of their overlap multiplied by `A`.
   *
   * @param x Pointer to an array whose first element x[0] is the layer coordinate (continuous).
   * @param par Parameter array where par[0] = A (amplitude scale) and par[1] = mu (peak center).
   * @return double Overlap length between the two unit intervals multiplied by `A`; returns 0.0 when there is no overlap. 
   */
  ...
  /**
   * @brief Evaluates a scaled Gaussian in the phi dimension.
   *
   * Computes par[0] multiplied by a Gaussian evaluated at x[0] with mean par[1]
   * and standard deviation par[2]. If par[2] is less than 0, returns 0.0.
   *
   * @param x Pointer to an array where x[0] is the phi coordinate to evaluate.
   * @param par Parameter array where:
   *            - par[0]: amplitude (scale factor),
   *            - par[1]: mean,
   *            - par[2]: standard deviation (sigma).
   * @return double The value par[0] * Gaussian(x[0]; par[1], par[2]) or `0.0` if par[2] < 0.
   */
  ...
  /**
   * @brief Computes a modified Gaussian multiplied by an asymmetric complementary error-term factor for a time coordinate.
   *
   * Evaluates par[0] * Gaussian(x[0]; mean=par[1], sigma=par[2]) * (1 + erfc(par[3] * (x[0]-par[1])/(sqrt(2)*par[2]))).
   * If par[2] is negative, the function returns 0.0.
   *
   * @param x Pointer to the independent-variable array; x[0] is the time coordinate to evaluate.
   * @param par Parameter array where:
   *   - par[0]: overall amplitude scaling,
   *   - par[1]: Gaussian mean,
   *   - par[2]: Gaussian standard deviation (sigma); must be >= 0 or result is 0,
   *   - par[3]: factor controlling the complementary-error-term asymmetry.
   * @return double The evaluated value of the modified Gaussian at x[0].
   */
  ...
  /**
   * @brief Extracts the connected region of hits that contains a specified seed hit.
   *
   * Given a set of candidate hits, replaces clusHits with the subset that forms the
   * connected region (via 3D adjacency) containing the hit identified by maxKey.
   * Connectivity is determined by matching neighbor offsets in integer 3D coordinates
   * (coordinates compared within a tolerance of 0.01).
   *
   * @param clusHits On input, the list of candidate hits (each as (point, key)). On
   * output, contains only the hits belonging to the connected region that includes
   * the seed specified by maxKey.
   * @param maxKey Pair of (hitkey, hitsetkey) identifying the seed hit whose region
   * should be extracted; this parameter is not modified.
   */
  ...
  /**
   * @brief Remove a set of clustered hits from the spatial index and ADC lookup.
   *
   * For each hit in clusHits, removes the hit from the provided R-tree and
   * erases the first entry in adcMap whose spechitkey matches the hit's
   * spechitkey.
   *
   * @param clusHits Vector of hit records to remove; each element's spechitkey is used to find ADC entries.
   * @param rtree Spatial index (R-tree) containing the hits; matching entries will be removed.
   * @param adcMap Multimap mapping ADC values to hit keys; the first matching entry per hit will be erased.
   */
  ...
  /**
   * @brief Compute and record cluster properties from a set of hits.
   *
   * Builds a LaserClusterv2 describing the provided hits, computes its geometric
   * and statistical properties (ADC sum, centroid in layer/iphi/time and in x/y/z,
   * per-axis spreads, and counts of distinct layer/iphi/it bins), and appends
   * the cluster and its generated cluster key to the thread_data vectors.
   * When enabled, a constrained 3D fit refines the cluster position and shape;
   * otherwise ADC-weighted centroids are used. Temporary histograms and fit
   * objects are internally created and cleaned up.
   *
   * @param clusHits Vector of hit entries that belong to the cluster (coordinates + ADC + hit keys).
   * @param my_data Per-thread context and storage; updated by appending the new cluster and its key.
   * @param maxADCKey Hit identifier used as the seed/key basis for generating the cluster key.
   */
  ...
  /**
   * @brief Build spatial index from the thread's hitsets and extract clusters for the assigned module region.
   *
   * Processes the hits listed in my_data->hitsets: filters hits by ADC threshold and proximity to the thread's peak time bin,
   * inserts unique hits into a spatial index, then repeatedly finds connected hit regions, computes cluster parameters,
   * and removes processed hits until no candidates remain. Discovered clusters and their keys are appended to the
   * corresponding containers in the provided thread_data.
   *
   * @param my_data Per-thread processing state and configuration (geometry, thresholds, peak time, layer bounds, verbosity,
   *                and output cluster vectors). Must contain populated hitsets; returns immediately if hitsets is empty.
   */
  ...
  /**
   * @brief Thread entry point that processes a module's data.
   *
   * Processes the work described by the provided thread_data and then terminates the calling thread.
   *
   * @param threadarg Pointer to a thread_data struct containing per-thread processing state.
   * @return Always `nullptr`; the thread terminates by calling `pthread_exit`.
   */
 ...
 * @brief Construct a LaserClusterizer subsystem instance.
 *
 * @param name Subsystem name passed to the SubsysReco base class; used to identify this module in the run framework.
 */
...
/**
 * @brief Initialize runtime resources, ensure cluster and geometry nodes exist, and configure drift timing.
 *
 * Ensures the DST/TRKR node structure contains a LaserClusterContainer (creates one under TRKR if missing),
 * locates the TPC geometry container, and derives the ADC clock period and maximum drift time used by the clusterizer.
 *
 * @param topNode Root node of the event/node tree to search for DST and geometry nodes.
 * @return int Fun4AllReturnCodes::EVENT_OK on success, Fun4AllReturnCodes::ABORTRUN if required nodes (DST or TPC geometry) are missing.
 */
...
/**
 * @brief Process a single event: identify laser events, cluster TPC laser hits per sector/side/module, and add resulting clusters to the configured LaserClusterContainer.
 *
 * This routine:
 * - Verifies required nodes (EventHeader, LaserEventInfo, DST, TRKR_HITSET, ActsGeometry, and the laser cluster container).
 * - Skips events that are not identified as laser events according to LaserEventInfo and run-number rules.
 * - Partitions TPC hitsets by sector, side, and module, then launches worker threads (or runs sequentially) to build clusters for each partition.
 * - Collects per-thread clusters and adds them to the laser cluster container.
 *
 * @param topNode Root of the PHENIX/Fun4All node tree containing event and detector data.
 * @return int Fun4AllReturnCodes::EVENT_OK on success;
 *             Fun4AllReturnCodes::ABORTRUN if required nodes or geometry are missing or LaserEventInfo cannot be found;
 *             non-zero error code on thread or mutex initialization failures.
 */
