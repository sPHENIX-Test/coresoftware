...
/**
 * @brief Generate a Pythia8 event that satisfies the configured triggers, convert it to HepMC, insert it into the PH node tree, and update run statistics.
 *
 * This function repeatedly generates Pythia8 events until the registered trigger logic (OR/AND semantics) is satisfied,
 * then converts the accepted Pythia8 event into a HepMC::GenEvent, optionally appends the generator event weight,
 * inserts the HepMC event into the PH node tree via PHHepMCGenHelper, increments the internal event counter,
 * and updates the PHGenIntegral statistics (accepted event count, processed event count, sum of weights, and integrated luminosity)
 * when the integral node is present.
 *
 * @return Fun4AllReturnCodes::EVENT_OK on success, Fun4AllReturnCodes::ABORTRUN if insertion of the HepMC event into the PH node tree fails.
 */
