...
/**
 * Representation of a generated event in the HepMC event record.
 *
 * Forward declaration of HepMC::GenEvent; include the HepMC event headers for the full definition and functionality.
 */
namespace HepMC
{
  class GenEvent;
...
class HepMCParticleTrigger : public SubsysReco
{
  ...
  */
  ...
  /**
 * Restrict selection to stable particles only when enabled.
 * @param b `true` to enable filtering to stable particles only, `false` to include unstable particles.
 */
void SetStableParticleOnly(bool b) { m_doStableParticleOnly = b; }
 ...
 private:
  ...
  /**
 * @brief Number of events processed by this module.
 *
 * Incremented for each call to process_event to track how many events have been examined.
 */
int n_evts{0};
  /**
 * Number of events classified as "good" by the trigger.
 *
 * Incremented for each event that passes the configured particle-type and kinematic cuts.
 */
int n_good{0};
  ...
  /**
 * Enable applying both Pz high and low cuts.
 *
 * When true, particles are required to satisfy both the configured Pz high and Pz low thresholds.
 */
