 ...
 *
 * Forward declaration of `GenEvent` used to reference HepMC event objects (the container for generated particles and event-level information).
 */
namespace HepMC
{
  class GenEvent;
...
class HepMCParticleTrigger : public SubsysReco
{
  ...
  */
  ...
  /**
 * Restrict particle selection to stable particles when enabled.
 * @param b If `true`, only stable particles are considered; if `false`, both stable and unstable particles are allowed.
 */
void SetStableParticleOnly(bool b) { m_doStableParticleOnly = b; }
 ...
 private:
  ...
  /**
 * @brief Total number of events processed by this instance.
 *
 * Counts every event examined by process_event; used for bookkeeping and progress/statistics.
 */
int n_evts{0};
  /**
 * Number of events that met the trigger selection criteria.
 */
int n_good{0};
